...
所有非空时间戳必须带明确时区；`window_start <= window_end <= evaluated_at`，`observed_at <= evaluated_at`。来源缺失或不可达时，source/window 时间必须为 null，只允许 `Unknown`、空值、不可决策；权限不足和 authority 冲突必须以 `No-Go`、空值、不可决策表达。
...
失败优先级固定为 `No-Go > Unknown > Trusted`。权限失败、authority 冲突、质量失败或低于隐私阈值时，即使来源同时缺失或过期，也不得用 `Unknown` 隐藏更强失败；混合状态必须拒绝或按明确 No-Go 原因输出。
...
- G0 source-missing Unknown（source/window time null、value null）；
- G1 undersized No-Go（high-risk sample size 25、value null、不可用于决策）。
- G0 unauthorized No-Go 与 authority-conflict No-Go（value null、不可用于决策）。
