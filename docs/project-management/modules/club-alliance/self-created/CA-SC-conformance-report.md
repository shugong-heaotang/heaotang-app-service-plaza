...
- compatibility：SC 详情固定为 `/api/v1/clubs/self-created/:id`，负例拒绝收窄共享 `/api/v1/clubs/:id`。
- route ID：合同要求 canonical positive safe integer；禁止页面用 `Number()` 把非规范路径重解释为其他真实资源。
