...
- Major 根因修复：详情路由只接受 canonical 正安全整数；`1e2/0x65/+101/00101/0/unsafe integer` 均以 `CLUB_ID_INVALID` 失败关闭，detail/join 零调用。
- 详情读取已切换到 SC 专用 `GET /api/v1/clubs/self-created/:id`，不再调用通用详情端点。

## 已关闭的前置漂移

- 平台 T0 prerequisite `815d29cff13e9d0b52cfd802b35f67f34594e426` 已统一 shared-session、权威 category error、404 资源隐藏、内部 500、canonical ID 与 SC 专用详情端点；独立复审 17/17 通过。
...
3. 核对 canonical route ID 六类负例均为页面层零请求，并确认详情只命中 `/api/v1/clubs/self-created/:id`。
