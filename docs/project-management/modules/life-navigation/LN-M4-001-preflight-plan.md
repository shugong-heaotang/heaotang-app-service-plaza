...
- 浏览器登录必须经正式 `AuthPanel` 和配置的测试验证码渠道；页面不得显示后端验证码或 token。
...
- 强制账号方法：M4 继续只使用上述两个脱敏 test-only 身份；每次会话临时获取、仅内存持有并在步骤结束后清空。报告只记录脱敏标识、状态码、用户 ID 是否匹配和验证时间，不记录验证码、JWT 或完整手机号。任一身份探针失败立即恢复为 No-Go，不得开启公开开发验证码或把凭据写入文件作为绕过。
...
5. API 验收：未登录 401；用户 A 首次 201、同键重放 200 且原 ID/响应头一致、异载荷 409、本人 GET 可见；用户 B GET 不可见用户 A 记录；请求 ID 与错误码可追踪。
6. 浏览器 UAT：未登录登录交接、空/加载、提交、成功后历史刷新、页面刷新后历史仍在、错误恢复、返回 `/services`，覆盖目标移动和桌面视口。
