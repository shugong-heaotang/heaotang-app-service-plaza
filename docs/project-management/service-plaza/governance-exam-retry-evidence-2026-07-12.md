...
- 当前：R1 source `a8106033` 因静默自动重读证据被独立复核 No-Go；R2 source `f61f0eed` 因新 checklist/IR 文件末尾空行被真实 diff 门禁拒绝，历史保留但最终树不再包含其快照；R3 以全新 checklist/exam/IR 完成显式展示/确认修复，等待独立复核与受控集成。
