...
8. 检查单完成后参加随机治理考试，8 题必须全部答对；失败记录不可修改，补课后新建下一次考试。
9. 板块和重要切片作为独立子项目，由板块负责人实现；平台负责人下达接口、分段抽查并最终验收，不代做全部业务。
...
- 推荐使用 `$heaotang-project-preflight` Skill 执行读取、检查单、考试、工作项和阶段 Handoff 标准流程。
