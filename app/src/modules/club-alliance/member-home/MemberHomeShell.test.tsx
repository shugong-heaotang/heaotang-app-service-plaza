...
describe("MemberHomeShell",()=>{
 it("renders separated club and membership status before exploration",()=>{render(<MemberHomeShell model={ready}/>);const labels=[...document.querySelectorAll("[aria-label]")].map(x=>x.getAttribute("aria-label"));expect(labels.indexOf("我的俱乐部")).toBeLessThan(labels.indexOf("探索更多"));expect(screen.getByText("合成家庭俱乐部")).toBeInTheDocument();expect(screen.getByText(/俱乐部正常 · 会员有效/)).toBeInTheDocument();expect(screen.getAllByRole("link")).toHaveLength(7)});
 it("renders an optional suspended membership without treating it as club lifecycle",()=>{render(<MemberHomeShell model={{...ready,clubs:[{...ready.clubs![0],membershipStatus:"suspended"}]}}/>);expect(screen.getByText(/俱乐部正常 · 会员停权/)).toBeInTheDocument()});
